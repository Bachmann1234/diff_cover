...
class TestCloverXmlCoverageReporterTest:
    ...
    @pytest.fixture(autouse=True)
    def patch_git_patch(self, mocker):
        # Paths generated by git_path are always the given argument. The Clover
        # lookup resolves them in its own module, so patch both.
        for module in (
            "diff_cover.violationsreporters.violations_reporter",
            "diff_cover.violationsreporters.clover",
        ):
            _git_path_mock = mocker.patch(f"{module}.GitPathTool")
            _git_path_mock.relative_path = lambda path: path
            _git_path_mock.absolute_path = lambda path: path
    ...
    def test_file_lookup_is_built_once(self, mocker):
        ...
        relative_path = mocker.patch(
            "diff_cover.violationsreporters.clover.GitPathTool.relative_path",
    ...
    def test_file_without_a_path_is_skipped(self):
        # A `<file>` carries its path in `path`, or in `name` from PHPUnit's
        # writer. One with neither cannot be matched against a source path, and
        # it must not keep the rest of the report from being indexed.
        xml = self._coverage_xml(["file1.java"], self.FEW_VIOLATIONS, self.FEW_MEASURED)
        xml.find(".//package").insert(0, etree.Element("file"))

        coverage = XmlCoverageReporter([xml])

        assert coverage.violations("file1.java") == self.FEW_VIOLATIONS
        assert coverage.measured_lines("file1.java") == self.FEW_MEASURED
